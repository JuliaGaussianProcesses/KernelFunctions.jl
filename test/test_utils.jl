...
function test_ADs(kernelfunction, args=nothing; ADs=_DEFAULT_ADS, dims=[3, 3])
...
function test_ADs(k::MOKernel; ADs=_DEFAULT_ADS, dims=(in=3, out=2, obs=3))
...
"""
    test_ADs_DIT(kernelfunction, args=nothing; ADs=nothing, dims=[3, 3])

DIT-based version of `test_ADs`. Tests gradient correctness of kernel functions
across multiple AD backends using DifferentiationInterfaceTest.jl.

Uses FiniteDifferences as reference backend.
"""
function test_ADs_DIT(kernelfunction, args=nothing; ADs=nothing, dims=[3, 3])
    ...
    fd_ok = true
    @testset "FiniteDifferences (DIT)" begin
    ...
    @testset "AD correctness (DIT)" begin
...
"""
    test_ADs_DIT(k::MOKernel; ADs=nothing, dims=(in=3, out=2, obs=3))

DIT-based version of `test_ADs` for multi-output kernels.
"""
function test_ADs_DIT(k::MOKernel; ADs=nothing, dims=(in=3, out=2, obs=3))
    backends = _resolve_backends(ADs)
    rng = MersenneTwister(42)
