...
@inline kappa(κ::RationalQuadraticKernel, d²::T) where {T<:Real} = (one(T)+0.5d²/κ.α)^(-κ.α)
...
struct GammaRationalQuadraticKernel{T,Tr,Tα<:Real,Tγ<:Real} <: Kernel{T,Tr}
    ...
    α::Tα
    γ::Tγ
    function GammaRationalQuadraticKernel{T,Tr,Tα}(t::Tr,α::Tα,γ::Tγ) where {T,Tr,Tα<:Real,Tγ<:Real}
...
@inline kappa(κ::GammaRationalQuadraticKernel, d²::T) where {T<:Real} = (one(T)+0.5d²/κ.α)^(-κ.α)
