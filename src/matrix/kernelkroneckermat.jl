...
function kernelkronmat(κ::Kernel, X::AbstractVector, dims::Int)
    @assert iskroncompatible(κ) "The chosen kernel is not compatible with constructing Kronecker matrices (see [`iskroncompatible`](@ref))"
    k = kernelmatrix(κ, X)
...
"""
    To be compatible with Kronecker constructions the kernel must satisfy
